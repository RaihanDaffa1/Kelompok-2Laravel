...
<div class="max-w-2xl mx-auto bg-white p-6 rounded-lg shadow">
    <h2 class="text-xl font-semibold text-blue-700 mb-4">Tambah Guru</h2>
    ...
    <form action="{{ route('teacher.store') }}" method="POST" class="space-y-4">
        ...
        <div class="space-y-3">
            <div>
                ...
                <input type="text" name="nip" value="{{ old('nip') }}" class="w-full border rounded-lg p-2" required>
            </div>
            <div>
                <label class="block text-sm font-medium text-gray-700">Nama Lengkap</label>
                <input type="text" name="nama_lengkap" value="{{ old('nama_lengkap') }}" class="w-full border rounded-lg p-2" required>
            </div>
            <div>
